...
def train(model, train_loader, criterion, loss_fn, batch_size, optimizer, device, epoch, num_epochs):
    model.train()
    running_loss = 0.0
    total = 0
    ...
    if hasattr(model, 'eeg_areas'):
        correct_areas = [0 for _ in range(len(model.eeg_areas))]  # 8 brain regions
    ...
    for ef, batch in enumerate(train_loader_tqdm):

        inputs, labels = batch["eeg"].to(device), batch["label"].to(device)
        inputs = add_gaussian_noise(inputs, mean=0.0, std=0.01)
        ...
        # Update progress bar to show fused acc
        if ef % 10 == 0:
            train_loader_tqdm.set_postfix({
                "loss": f"{running_loss / (total / batch_size):.4f}",
                "fused_acc": f"{100 * correct_fused / total:.2f}%",
            })
        # train_loader_tqdm.set_postfix({
        #     "loss": f"{running_loss / (total / batch_size):.4f}",
        #     "fused_acc": f"{100 * correct_fused / total:.2f}%",
    ...
    print(f"\nEpoch {epoch} | Fused Acc: {fused_acc:.2f}% | All Acc: {all_acc:.2f}%")
    print(f"Loss: {loss.item():.4f} | α: {loss_dict['alpha']:.3f} | β: {loss_dict['beta']:.3f}")
    return epoch_loss, fused_acc, all_acc, area_accs, [conformer_acc]
